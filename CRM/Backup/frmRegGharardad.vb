Public Class frmRegGharardad
    ...
    Private Sub frmRegGharardad_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        ...
        If VahedNameID <> String.Empty Then
    ...
    Private Sub FillCboHeadGroup()
        ...
        Dim cmd2 As New SqlClient.SqlCommand("SELECT HeadGroup from GharardadNo Group By HeadGroup Order by HeadGroup", SqlCon.SqlCon)
        SqlCon.SqlCon.Open()
...
#End Region
    ...
    Private Sub SetSold()
        ...
        Cmd = New SqlClient.SqlCommand
        Cmd.Connection = SqlCon.SqlCon
        Cmd.CommandText = "Update VahedName set Sold='1' Where ID='" & VahedNameID & "'"
        SqlCon.SqlCon.Open()
        Try
            Cmd.ExecuteNonQuery()
        Catch ex As Exception
    ...
    Private Sub Timer_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Timer.Tick
        ...
        Dim S1 As Integer = GetSum.GetSum(VahedNameID, "پیش پرداخت")
        Dim S2 As Integer = GetSum.GetSum(VahedNameID, "اقساط")
        Dim S3 As Integer = GetSum.GetSum(VahedNameID, "نقدی")
    ...
    Private Sub FillTextBox(ByVal VahedNameID As String)
        ...
        PG.Generate(VahedNameID)
        '------------- sms ------------
        FillCboDate(MobileNumber)
        FillDbgReport(MobileNumber)
        '------------------
        AddKharidarName()
        '-------------------------
        FillCboBedehiSallerName()
        FillDbgBedehi()
        '-----------------------------
        CountMetrajPrice()
    ...
    Private Sub BedehiSayer()
        Cu = New Currency
        NtoW = New NumberToWord
        Me.txtBedehiSayer.Text = Cu.CodeNumber(GetSum.GetSumPardakhtNaShode(VahedNameID, "سایر پرداخت ها")) & "    معادل    " & NtoW.Convert(GetSum.GetSumPardakhtNaShode(VahedNameID, "سایر پرداخت ها"))
    End Sub
    ...
    Private Sub CheckVaziyat()
        If Me.btnTaidiye.BackColor = Color.Red Then
            Me.TabNoeKharidar.TabPages(8).Text = "وضعیت = تایید نشده"
        Else
            Me.TabNoeKharidar.TabPages(8).Text = "وضعیت = تایید شده"
        End If
    End Sub
    ...
    Private Sub UpdateTozihat()
        If Me.btnReg.Enabled = False Then
            Dim SqlCon As New SQL
            Dim Cmd As New SqlClient.SqlCommand
            Cmd = New SqlClient.SqlCommand
            Cmd.Connection = SqlCon.SqlCon
            Cmd.CommandText = "Update GharardadNo Set MobayeAmlaak='" & RbMobayeChecked() & "', MobayeDaftar='" & Me.chkMobayeDaftar.Checked & "', Shora='" & Me.chkMohrShora.Checked & "', Amlaak='" & Me.chkMohrAmlaak.Checked & "', Forooshande1='" & Me.chkForooshande1.Checked & "', Forooshande2='" & Me.chkForooshande2.Checked & "', Rahgiri='" & Me.chkRahgiri.Checked & "', EmzaKharidar='" & Me.chkEmzaKharidar.Checked & "',Taidiye='" & Taidiyeh() & "' where VahedNoID='" & VahedNameID & "'"
    ...
    Private Sub btnUpdateMertaj_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUpdateMertaj.Click
        If IsNumeric(Me.txtMetrajeZamin.Text) = True Then
            Dim SqlCon As New SQL
            Dim Cmd As New SqlClient.SqlCommand
            Cmd = New SqlClient.SqlCommand
            Cmd.Connection = SqlCon.SqlCon
            Cmd.CommandText = "Update VahedName Set MetrajZamin='" & Me.txtMetrajeZamin.Text & "' where ID='" & VahedNameID & "'"
            ...
            Try
                ...
                'RefreshText()
            Catch ex As Exception
                MessageBox.Show(ex.Message.ToString, "خطا:::... Update Metraj Vahed", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1, MessageBoxOptions.RightAlign)
            Finally
                SqlCon.SqlCon.Close()
            End Try
        End If
    ...
    Private Sub FilldbgSayerPardakhtHa()
        ...
        Dim SQLCONection As New SQL
        Dim V As String = String.Empty
        Dim cmd2 As New SqlClient.SqlCommand("Select Row as 'شماره سند',FormType as 'نحوه دریافت',Jahate as 'دریافت جهت',Price as 'مبلغ دریافت',Date as 'تاریخ سررسید',PardakhtDate as 'تاریخ دریافت',Passed as 'وضعیت دریافت' From Bank Where ID='" & VahedNameID & "' And Pardakhte=N'سایر پرداخت ها'", SQLCONection.SqlCon)
        Try
            SQLCONection.SqlCon.Open()
    ...
    Private Sub FilldbgAllPardakht()
        ...
        Try
            Cmd.CommandText = "Select Row as 'شماره سند',FormType as 'نحوه دریافت',Pardakhte as 'نوع پرداخت',Jahate as 'دریافت جهت',Price as 'مبلغ دریافت',Date as 'تاریخ سررسید',PardakhtDate as 'تاریخ دریافت',Passed as 'وضعیت دریافت' From Bank Where ID='" & VahedNameID & "' And Pardakhte!=N'سایر پرداخت ها' Order by Pardakhte,FormType,Date"
            SqlCon.SqlCon.Open()
            Sdr = Cmd.ExecuteReader
            Dim fc As Integer
            While (Sdr.Read)
    ...
    Private Sub dbgAllPardakht_CellMouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dbgAllPardakht.CellMouseDoubleClick
        ...
        BedehiSayer()
    End Sub
    Private Sub FillDbgReport(ByVal MobileNumber As String)
        Dim SqlCon As New SQL
        Dim Cmd As New SqlClient.SqlCommand
        Dim Sdr As SqlClient.SqlDataReader
        Dim tbl As New DataTable
        ...
        If Me.cboSendType.Text <> "کل پیامک ها" Then
            ParentType = " Parent=N'" & Me.cboSendType.Text & "' And "
            PType = String.Empty
        Else
            PType = " S.Parent as 'عنوان پیامک',"
        End If
        ...
        Try
            Cmd.CommandText = "Select  Row_Number() Over(order by S.[Time] Desc) as 'ردیف'," & PType & " G.GharardadNo as 'شماره قرارداد',G.Name + ' ' + G.Lname as 'نام مشتری',S.Messagetext as 'متن پیامک',S.MobileNumbers as 'شماره همراه مشتری',S.[Time] as 'زمان ارسال',S.Sender as 'شماره فرستنده',S.Sent as 'شرح وضعیت ارسال' from GharardadNo G Inner Join SmsSendMessages S On G.VahedNoID=S.UserID where " & ParentType & " SendOK='" & Me.cboReportType.SelectedIndex & "' AND date=N'" & Me.cboDate.Text & "' AND mobileNumbers=N'" & MobileNumber & "'"
            SqlCon.SqlCon.Open()
            Sdr = Cmd.ExecuteReader
            Dim fc As Integer
            While (Sdr.Read)
            ...
            Me.dbgSMS.DataSource = dvw

            '-------------------------

        Catch ex As Exception
            'MessageBox.Show(ex.Message.ToString, " خطا در خواندن اطلاعات پیامک", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1, MessageBoxOptions.RightAlign)
        Finally
            SqlCon.SqlCon.Close()
        End Try
    End Sub

    Private Sub FillCboDate(ByVal MobileNumber As String)
        Dim SqlCon As New SQL
        Dim Cmd As New SqlClient.SqlCommand
        Dim Sdr As SqlClient.SqlDataReader
        ...
        SqlCon = New SQL
        Dim cmd4 As New Data.SqlClient.SqlCommand("select Date from smsSendMessages group by Date,MobileNumbers having MobileNumbers=N'" & MobileNumber & "' order by Date desc", SqlCon.SqlCon)
        SqlCon.SqlCon.Open()
    ...
    Private Sub dbgSMS_CellMouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dbgSMS.CellMouseDoubleClick
        If Me.cboSendType.Text <> "کل پیامک ها" Then
            Me.txtSMStext.Text = Me.dbgSMS.CurrentRow.Cells.Item(3).Value.ToString()
        Else
            Me.txtSMStext.Text = Me.dbgSMS.CurrentRow.Cells.Item(4).Value.ToString()
        End If
    End Sub

    Private Sub btnSendSMS_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSendSMS.Click
        If Me.txtSMStext.Text <> String.Empty And Me.txtSMStext.Text.Length > 10 Then
            Try
                Dim GSCI As New GetSmsCenterInfo
                Dim FD As New FillData
                Dim IDD As String = Guid.NewGuid.ToString
                Dim Send As New SendSMS
                FD.SaveData(IDD, GSCI.GetSMSAghsatInfo(3), Me.txtSMStext.Text, Me.txtHaghMobile.Text, CD.GetDay.ToString, Now.TimeOfDay.ToString, VahedNameID, "اطلاع رسانی")
                '-----------------------------------------------------
                Send.Send(IDD, Me.txtHaghMobile.Text, GSCI.GetSMSAghsatInfo(3), Me.txtSMStext.Text, "Normal")
                '--------------------------
                Dim SqlCon As New SQL
                Dim Cmd As New SqlClient.SqlCommand
                Dim Sdr As SqlClient.SqlDataReader
                ...
                SqlCon = New SQL
                Dim cmd4 As New Data.SqlClient.SqlCommand("Select Sent from SmsSendMessages Where ID='" & IDD & "'", SqlCon.SqlCon)
                SqlCon.SqlCon.Open()
                Dim sdr4 As Data.SqlClient.SqlDataReader = cmd4.ExecuteReader
                While sdr4.Read
                    SentLog = sdr4(0).ToString
                End While
                SqlCon.SqlCon.Close()
                sdr4.Close()
                ...
                MessageBox.Show(SentLog, "ارسال پیامک", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1, MessageBoxOptions.RightAlign)
            Catch ex As Exception
                MessageBox.Show("خطای نا مشخص رخ داده است" & vbCrLf & ex.Message.ToString, "ارسال پیامک", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1, MessageBoxOptions.RightAlign)
            End Try
        Else
            MessageBox.Show("لطفاً متن پبامک را مشخص نمایید", "خطا در اطلاعات ", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1, MessageBoxOptions.RightAlign)
        End If

        FillDbgReport(MobileNumber)
    End Sub

    Private Sub AddKharidarName()
        Dim Gender As String = String.Empty
        If Me.cboGenderHagh.Text = "آقای" Then
            Gender = "جناب آقای "
        Else
            Gender = "سرکار خانم "
        End If

        Me.txtSMStext.Text = Gender & Me.txtHaghName.Text & " " & Me.txtHaghLname.Text & vbCrLf & "خریدار محترم پروژه " & Me.txtMajmooeName.Text & vbCrLf & Me.txtSMStext.Text
    End Sub

    Private Sub btnAddKharidarName_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAddKharidarName.Click
        AddKharidarName()
    End Sub

    Private Sub FillCboBedehiSallerName()
        Me.cboBedehiSallerName.Items.Clear()
        Me.cboBedehiSallerName.Items.Add(Me.cboGenderHagh.Text & " " & Me.txtHaghName.Text & " " & Me.txtHaghLname.Text)
        Me.cboBedehiSallerName.Text = Me.cboGenderHagh.Text & " " & Me.txtHaghName.Text & " " & Me.txtHaghLname.Text

        If Me.cboHeadGroup.Text <> "" Then
            Me.cboBedehiSallerName.Items.Add("کل گروه " & Me.cboHeadGroup.Text)
        End If
        '-------------------------------
        CD = New CurrentDate
        Me.txtBedehiDate.Text = CD.GetDate
        '-------------------------------
    End Sub

    Private Sub FillDbgBedehi()
        NtoW = New NumberToWord
        Dim SqlCon As New SQL
        Dim Cmd As New SqlClient.SqlCommand
        Dim Sdr As SqlClient.SqlDataReader
        ...
        Dim SumBedehiPrice As Integer
        Dim NameMoshtari As String = String.Empty

        Select Case Me.cboBedehiReportType.Text
            Case Is = "پرداخت نشده"
                Shart = " And Passed='0'"
            Case Is = "پرداخت شده"
                Shart = " And Passed='1'"
        ...
        If Me.cboBedehiSallerName.SelectedIndex = 0 Then
            Shart += " AND B.ID='" & VahedNameID & "' "
        Else

            SqlCon = New SQL
            Dim cmd4 As New Data.SqlClient.SqlCommand("Select VahedNoID from GharardadNo Where HeadGroup=N'" & Me.cboHeadGroup.Text & "'", SqlCon.SqlCon)
            SqlCon.SqlCon.Open()
            Dim sdr4 As Data.SqlClient.SqlDataReader = cmd4.ExecuteReader
            While sdr4.Read
                GroupIDs += "B.ID='" & sdr4(0).ToString & "' OR "
            End While
            SqlCon.SqlCon.Close()
            sdr4.Close()
            If GroupIDs.Length > 5 Then
                Shart += " AND (" & Mid(GroupIDs, 1, GroupIDs.Length - 3) & ")"
            End If
            NameMoshtari = ",G.Name + ' ' + G.Lname as 'نام و نام خانوادگی',G.GharardadNo as 'شماره قرارداد'"
        End If
        ...
        Try
            '                                     0                                           1                        2                         3                      4                  5                     6                   7                   
            Cmd.CommandText = "Select Row_Number() Over(order by Date Desc) as 'ردیف'" & NameMoshtari & ",B.Row as 'شماره سند',B.FormType as 'نحوه دریافت',B.Pardakhte as 'نوع پرداخت',B.Jahate as 'دریافت جهت',B.Price as 'مبلغ دریافت',B.Date as 'تاریخ سررسید',B.Passed as 'وضعیت دریافت' From Bank B Inner join GharardadNo G on B.ID=G.VahedNoID Where  B.Date < N'" & Me.txtBedehiDate.Text & "' " & Shart & " order by B.Date Desc"
            SqlCon.SqlCon.Open()
            Sdr = Cmd.ExecuteReader
            Dim fc As Integer = 0
            While (Sdr.Read)
                ...
                If Me.cboBedehiSallerName.SelectedIndex = 0 Then
                    ...
                    row(3) = Sdr(3)
                    row(4) = Sdr(4)
                    row(5) = Cu.CodeNumber(Sdr(5))
                    SumBedehiPrice += Sdr(5)
                    row(6) = Sdr(6)
                ...
                Else
                    ...
                    row(5) = Sdr(5)
                    row(6) = Sdr(6)
                    row(7) = Cu.CodeNumber(Sdr(7))
                    SumBedehiPrice += Sdr(7)
                    row(8) = Sdr(8)
            ...
            End While
            dvw = New Data.DataView(tbl)
            Me.DbgBedehi.DataSource = dvw

            '-------------------------
            Me.lblBedhiPrice.Text = Cu.CodeNumber(SumBedehiPrice) & "  معادل  " & NtoW.Convert(SumBedehiPrice)
        Catch ex As Exception
            MessageBox.Show(ex.Message.ToString, " خطا در خواندن اطلاعات اقساط", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1, MessageBoxOptions.RightAlign)
        Finally
            SqlCon.SqlCon.Close()
        End Try
    End Sub

    Private Sub DbgBedehi_CellMouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles DbgBedehi.CellMouseDoubleClick
        Dim CBO As New ComboBox
        Dim Pr As String = 0
        CBO.Text = Me.DbgBedehi.CurrentRow.Cells.Item(2).Value.ToString()
        Select Case Me.DbgBedehi.CurrentRow.Cells.Item(3).Value.ToString()
            Case Is = "پیش پرداخت"
                Pr = Me.txtPishpardakht.Text
            Case Is = "اقساط"
                Pr = Me.txtAghsat.Text
        End Select
        CboSelector(CBO, Pr, Me.DbgBedehi.CurrentRow.Cells.Item(3).Value.ToString(), False, True)
        FillDbgBedehi()
        BedehiSayer()
    End Sub

    Private Sub cboBedehiSallerName_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboBedehiSallerName.SelectedIndexChanged
        FillDbgBedehi()
    End Sub

    Private Sub cboBedehiReportType_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboBedehiReportType.SelectedIndexChanged
        FillDbgBedehi()
    End Sub

    Private Sub txtBedehiDate_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtBedehiDate.KeyPress
        If e.KeyChar = Chr(13) Then
            FillDbgBedehi()
        End If
    End Sub

    Private Sub txtBedehiDate_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtBedehiDate.GotFocus
        Try
            Dim DM As New DateMode
            Me.txtBedehiDate.Text = DM.DECodeDate(Me.txtBedehiDate.Text)
        Catch ex As Exception
            txtBedehiDate.Focus()
        End Try
    End Sub

    Private Sub txtBedehiDate_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtBedehiDate.LostFocus
        Try
            Dim DM As New DateMode
            Me.txtBedehiDate.Text = DM.CodeDate(Me.txtBedehiDate.Text)
        Catch ex As Exception
            txtBedehiDate.Focus()
        End Try
    End Sub

    Private Sub txtGharardadPrice_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtGharardadPrice.TextChanged
        CountMetrajPrice()
    End Sub

    Private Sub CountMetrajPrice()
        Try
            Cu = New Currency
            Me.txtMetrPrice.Text = Cu.DeCodeNumber(Me.txtGharardadPrice.Text) \ Me.txtMetrajeZamin.Text
            Me.txtMetrPrice.Text = Cu.CodeNumber(Me.txtMetrPrice.Text)
        Catch ex As Exception
